...
RSpec.describe FetchEventfindaJob, type: :job do
  ...
  describe :perform do
    ...
    it "fetches a single page of results from the eventfinda api" do
      stub_request(:get, %r{http://nope}).to_return_json(body: { events: [{}, {}], :@attributes => { count: 2 } })
    ...
    it "fetches multiple pages of results from the eventfinda api" do
      stub_request(:get, %r{http://nope}).to_return_json(body: { events: [{}, {}], :@attributes => { count: 4 } })
      FetchEventfindaJob.new.perform
      upload = Lml::Upload.filter_by_source("eventfinda_com_au").order(created_at: :desc).limit(1)
      content = JSON.parse(upload[0].content, symbolize_names: true)
      expect(content).to eq({ events: [{}, {}, {}, {}] })
