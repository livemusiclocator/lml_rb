...
RSpec.describe Lml::GooglePlacesApiClient do
  before do
    ...
    @client = Lml::GooglePlacesApiClient.new(api_key: "test-key", initial_delay: 0)
  end

  describe "#find" do
    it "posts the query and returns the parsed response" do
      request = stub_request(:post, Lml::GooglePlacesApiClient::FIND_URL).with(
        body: { textQuery: "The Espy, St Kilda", regionCode: "AU" },
        headers: {
          "X-Goog-Api-Key" => "test-key",
          "X-Goog-FieldMask" => Lml::GooglePlacesApiClient::FIND_FIELDS,
    ...
    it "raises with the body, because an api key problem is only explained there" do
      ...
      expect { @client.find("The Espy") }
        .to raise_error(Lml::GooglePlacesApiClient::Error, /403.*has not been used in this project/m)
    end
  end
  ...
  describe "retrying" do
    ...
    it "gives up after the attempt cap rather than sitting on a daily quota" do
      request = stub_request(:post, Lml::GooglePlacesApiClient::FIND_URL).to_return(status: 503, body: "unavailable")

      expect { @client.find("The Espy") }.to raise_error(Lml::GooglePlacesApiClient::Error, /503/)

      expect(request).to have_been_requested.times(Lml::GooglePlacesApiClient::MAX_ATTEMPTS)
    end
    ...
    it "does not retry a fault that waiting cannot fix" do
      request = stub_request(:post, Lml::GooglePlacesApiClient::FIND_URL).to_return(
        status: 403,
        body: "Places API (New) has not been used in this project before",
      )

      expect { @client.find("The Espy") }.to raise_error(Lml::GooglePlacesApiClient::Error, /403/)

      expect(request).to have_been_requested.once
    end

    it "backs off further with each attempt" do
      stub_request(:post, Lml::GooglePlacesApiClient::FIND_URL).to_return(status: 503, body: "unavailable")
      client = Lml::GooglePlacesApiClient.new(api_key: "test-key", initial_delay: 1)
      slept = []
      allow(client).to receive(:sleep) { |seconds| slept << seconds }

      expect { client.find("The Espy") }.to raise_error(Lml::GooglePlacesApiClient::Error)

      expect(slept.length).to eq(Lml::GooglePlacesApiClient::MAX_ATTEMPTS - 1)
  ...
  describe "#details" do
    it "asks for one place by id" do
      stub_request(:get, Lml::GooglePlacesApiClient::DETAILS_URL.sub("%<place_id>s", "place-espy"))
